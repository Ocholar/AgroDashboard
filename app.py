...
def update_dashboard(sel_country, sel_season, sel_variety, sel_rain):
    dff = df.copy()
    if sel_country != "All":
    ...
    if sel_variety != "All":
        dff = dff[dff["Variety"] == sel_variety]
    if sel_rain != "All":
...
df["model_lon"] = pd.to_numeric(df["model_lon"], errors="coerce")
jitter = 0.0002
df["model_lat"] += np.random.uniform(-jitter, jitter, size=len(df))
df["model_lon"] += np.random.uniform(-jitter, jitter, size=len(df))

# Fill missing sample yield
df["AvgSampleYield_per_m2"] = df["AvgSampleYield_per_m2"].fillna(0.1)
...
def update_dashboard(selected_country, selected_season, selected_variety, selected_rain):
    # Apply filters
    dff = df.copy()
    if selected_country != "All":
        dff = dff[dff["Country"] == selected_country]
    if selected_season != "All":
        dff = dff[dff[season_col] == selected_season]
    if selected_variety != "All":
        dff = dff[dff["Variety"] == selected_variety]
    if selected_rain != "All":
        dff = dff[dff["RainType"] == selected_rain]

    # Drop invalid coords and yields
    dff = dff.dropna(subset=["model_lat", "model_lon", "YieldPerAcre"])
    ...
    height_px = max(600, len(bar_df) * 25)
    bar_fig = px.bar(
        bar_df,
        x="YieldPerAcre",
        y="DisplayName",
    ...
    bar_fig.update_layout(yaxis=dict(dtick=1))
    bar_chart = dcc.Graph(figure=bar_fig, style={"height": f"{height_px}px", "width": "100%"})

    # Top 5 varieties
    top5 = bar_df.sort_values("YieldPerAcre", ascending=False).head(5)
    top_varieties = html.Div([
        html.H4("Top 5 Varieties by Yield"),
        html.Ul([html.Li(f"{r['DisplayName']}: {r['YieldPerAcre']:.1f} kg/acre")
    ...
    country_yields = html.Div([
        html.H4("Country-Level Average Yield"),
        html.Ul([html.Li(f"{r['Country']}: {r['YieldPerAcre']:.1f} kg/acre")
                 for _, r in country_avg.iterrows()])
    ])
    ...
    rain_yields = html.Div([
        html.H4("Yield by Rain Type"),
        html.Ul([html.Li(f"{r['RainType']}: {r['YieldPerAcre']:.1f} kg/acre")
                 for _, r in rain_avg.iterrows()])
    ])
